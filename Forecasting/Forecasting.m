...
%         len_short = sum_len(ii) - sum_len_pred;
        ...
        if jj>40 && jj<47
            if jj ==41 % head segment length plus
                vec_len_pred(end) = vec_len_pred(end)+4;
            elseif jj ==42 % head segment length minus
                vec_len_pred(end) = max(vec_len_pred(end)-4,1);
            elseif jj ==43  % tail segment length plus
                vec_len_pred(1) = vec_len_pred(1)+4;
            elseif jj == 44  % tail segment length minus
                vec_len_pred(1) = max(vec_len_pred(1)-4,1);
            elseif jj == 45  % all segment length plus
                vec_len_pred = vec_len_pred + 4/length(vec_len_pred);
            else             % all segment length minus
                vec_len_pred = max(vec_len_pred - 4/length(vec_len_pred),1);  
            end
        end
