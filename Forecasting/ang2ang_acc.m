...
%% Artificially change the angle according to 'jj'
    ...
    elseif jj<41; % jj = 23~40, change omega randomly
        if jj == 23; % change a point near central
            ang_hypo_temp = ang;
            ang_hypo_temp(mid_start-2: mid_start-1) = ang_hypo_temp(mid_start-2: mid_start-1) + sign(randn(1))*[0.2; -0.2];
        elseif jj == 24; % change a point near central
            ang_hypo_temp = ang;
    ...
    elseif jj<47; % jj = 41~46, change lengths but keep angle the same
        ang_hypo_temp = ang;
    elseif jj == 47; % jj = 47, change two angles besides the middle point
        ang_hypo_temp = mid_pt_chg(ang,mid_start,1);
    elseif jj == 48; % jj = 48, change two angles besides middle point
        ang_hypo_temp = mid_pt_chg(ang,mid_start,-1);
